...
P3-B ✅ Fame/인기(2026-06-29): 승·KO·역전·이변·스펙터클(clean/knockdown)로 명성·인기 누적 → 이벤트 매치 흥행지수(라이벌리×인기). *crowd 적분 export는 추후 정교화 — v1은 MatchResult 신호로 근사.*
P3-C ✅ 저장/로드(2026-06-29, `world.json`): 커리어·명성·관계 스냅샷 저장/로드 → season 재실행 시 다시즌 누적. constantsVer 박제·불일치 경고. 검증: 3시즌 후 통산 104-22·관계 통산 0-18 심화.
P3-D ◐ 매치메이킹: **자동 편성 구현(2026-06-29)** — 정규시즌 뒤 흥행지수(라이벌리×인기) 상위 카드를 이벤트 빅매치로 자동 개최(커리어·명성·서사 반영, 시즌순위 무관). **플레이어 간접 개입(대진 직접 편성)은 보류** — 자동 위에 나중에.
```
