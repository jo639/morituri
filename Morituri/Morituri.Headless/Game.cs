...
public sealed partial class Game
{
    ...
    private static string[] PersonaQuotes(string pid) => pid switch
    {
        ...
        "PER_BOLD"      => new[] { "물러설 자리는 안 봐 뒀다.", "난 아직 끝나지 않았다.", "한 번에 끝내자.", "네가 세게 나올수록 좋다." },
    ...
    {
        // [13a] 카토를 내치면 내 선수의 훈련 주기가 한 경기 늘어난다 — 가르칠 사람이 없어졌으니.
        // 씬의 선택지가 약속한 대가다(약속만 하고 구현이 없던 것을 채운다). AI는 제 교관이 있으므로 무관.
        int cycle = TrainEveryMatches + (g.IsPlayer && _storyFlags.Contains("cato_exiled") ? 1 : 0);
        if (++g.MatchCounter < cycle) return 0;
        g.MatchCounter = 0;
        int pts = g.IsPlayer ? _trainingLv : 1;
        if (g.IsPlayer) { g.TrainingPoints += pts; return pts; }      // 라니스타가 분배
    ...
    private static string ArticleBody(string kind, SimRandom rng)
    {
        ...
        {
            "death" => new[] { "시신은 경기 직후 옮겨졌다. 관중석은 잠깐 조용했고, 다음 경기가 시작되자 다시 찼다.", "선술집 세 곳에서 그 이름이 나왔다. 대부분은 배당 이야기였다." },
            "promote" => new[] { "1부 승격이 확정됐다. 다음 시즌 일정은 곧 나온다.", "승격 소식이 돌자 후원 문의가 늘었다고 한다." },
            "relegate" => new[] { "2부로 내려간다. 급여와 출전료가 함께 내려간다.", "강등이 확정된 날 그 집 앞에 채권자가 왔다는 말이 있다." },
            "cup" => new[] { "챔피언십 컵의 주인이 바뀌었다. 결승 관중은 올해 최다였다.", "우승 상금과 함께 다음 시즌 출전료가 오른다." },
            "season" => new[] { "시즌이 끝났다. 순위에 따라 상금과 급여가 정산된다.", "겨우내 이적 문의가 오간다. 값은 이때 정해진다." },
            "upset" => new[] { "배당판이 뒤집혔다. 도박장 세 곳이 그날 정산을 미뤘다.", "이변 뒤에는 늘 재대결 요구가 붙는다. 표가 팔리기 때문이다." },
            "comeback" => new[] { "쓰러지기 직전에서 뒤집었다. 관중은 일어선 채로 끝을 봤다.", "경기 뒤 의무실이 붐볐다. 이긴 쪽도 실려 갔다." },
            "revenge" => new[] { "지난 패배를 갚았다. 두 사람의 상대전적이 뒤집혔다.", "재대결 요구가 벌써 붙었다. 흥행주들이 값을 매기는 중이다." },
            "persona" => new[] { "그날 이후 훈련 방식을 바꿨다는 말이 돈다.", "같은 사람이 다르게 싸운다는 이야기가 여러 곳에서 나왔다." },
            "debut" => new[] { "데뷔전을 치렀다. 아직 이름을 외운 사람은 많지 않다.", "신예 하나가 처음 모래를 밟았다. 다음 일정은 아직 나오지 않았다." },
            "recruit" => new[] { "노예 시장이 분주하다. 값은 지난달보다 올랐다." },
            "sparring" or "camp" => new[] { "프리시즌 훈련이 진행 중이다. 이 경기들은 기록에 남지 않는다." },
            "unrest" => new[] { "거리 순찰이 늘었다. 경기장 입구에서도 검문이 있었다." },
            "legend" => new[] { "한 이름이 전설 명부에 올랐다. 명부는 기록실 벽에 걸린다." },
            "injury" or "perm_injury" => new[] { "의원(醫院)으로 실려 갔다. 복귀 시기는 아직 말이 없다." },
            "patron" => new[] { "관람석에서 후원 이야기가 오갔다. 액수는 알려지지 않았다." },
            "retire" => new[] { "검을 내려놓았다. 빈자리는 신인이 받는다." },
            "match" => new[] { "판정까지 갔다. 관중은 대체로 만족했다.", "심판이 손을 들었다. 다음 경기 준비가 곧 시작됐다.", "짧게 끝났다. 표값 이야기가 나왔다." },
            _ => new[] { "특별한 일은 없었다." },
        };
