...
internal static class Program
{
    ...
    private static void Main()
    {
        ...
        int port = ViewerServer.StartBackground(AppContext.BaseDirectory, 5173, (method, path, body) => path switch
        {
            ...
            "/api/gacha" when method == "POST" => game.GachaJson(BoolOf(body ?? "", "premium")),
            ...
            "/api/retire" when method == "POST" => game.RetireJson(StrOf(body ?? "", "id"), StrOf(body ?? "", "path")),
            "/api/bestow" when method == "POST" => game.BestowJson(StrOf(body ?? "", "id"),
                                                                   StrOf(body ?? "", "traitId"), StrOf(body ?? "", "tacticId")),
            ...
            "/api/presecup" when method == "POST" => game.PreseasonCupJson(),
            "/api/banquet" when method == "POST" => game.BanquetJson(),
            "/api/manumit" when method == "POST" => game.ManumitJson(StrOf(body ?? "", "id")),
            "/api/live" when method == "POST" => game.LiveBeginJson(StrOf(body ?? "", "tacticId") is { Length: > 0 } lt ? lt : null,
                                                                    StrOf(body ?? "", "tacticB") is { Length: > 0 } ltb ? ltb : null,
                                                                    StrOf(body ?? "", "prep")),
            "/api/surgery" when method == "POST" => game.SurgeryJson(StrOf(body ?? "", "id"),
                                                                     StrOf(body ?? "", "kind") is { Length: > 0 } sk ? sk : "temp",
                                                                     StrOf(body ?? "", "part")),
            "/api/learnskill" when method == "POST" => game.LearnSkillJson(StrOf(body ?? "", "id"), StrOf(body ?? "", "skillId"),
                                                                           StrOf(body ?? "", "forget") is { Length: > 0 } fg ? fg : null),
            "/api/liveswitch" when method == "POST" => game.LiveSwitchJson(FloatOf(body ?? "", "time"), StrOf(body ?? "", "tacticId"), IntOf(body ?? "", "side")),
            "/api/settle" when method == "POST" => game.LiveSettleJson(),
            "/api/sparring" when method == "POST" => game.SparringJson(StrOf(body ?? "", "id")),
            "/api/streetfight" when method == "POST" => game.StreetFightJson(StrOf(body ?? "", "id"), StrOf(body ?? "", "target")),
            "/api/streettargets" when method == "POST" => game.StreetTargetsJson(),
            "/api/gangbrawl" when method == "POST" => game.GangBrawlJson(StrOf(body ?? "", "id")),
